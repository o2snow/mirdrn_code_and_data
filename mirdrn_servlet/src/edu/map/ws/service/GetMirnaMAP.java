...
public class GetMirnaMAP {
	...
	public String getByGet(@QueryParam(IVoConstants.PERMISSION_TOKEN_NAME) String token,
			@QueryParam(IVoConstants.REQUEST_MIRNA) String mirna,
			...
			try {
				...
				// Service
				ArrayList<VoTarbaseV5Uni> genelist = ws.GetListOfGeneByMirna(mirna);
				if(genelist != null && genelist.size() > 0){
					...
					for(VoTarbaseV5Uni vo : genelist){
						miRnaList.put(vo.getMiRna(), vo.getMiRnaSeq());
						geneIdList.put(vo.getGeneId(), vo.getOfficialSymbol());
						pubmedList.put(vo.getPimd(), vo.getPimd());
					}
