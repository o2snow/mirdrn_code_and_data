...
public class GetSirnaMAP {
	...
	public String getByGet(@QueryParam(IVoConstants.PERMISSION_TOKEN_NAME) String token,
			@QueryParam(IVoConstants.REQUEST_SIRNA) String sirna,
			...
			try {
				...
				// Service
				ArrayList<VoSirna> genelist = ws.GetListOfGeneBySirna(Integer.valueOf(sirna));
				if(genelist != null && genelist.size() > 0){
															
					// Associated miRNAs
					LinkedHashMap<Integer, String> miRnaList = new LinkedHashMap();
					LinkedHashMap<Integer, String> geneIdList = new LinkedHashMap();
					LinkedHashMap<String, String> pubmedList = new LinkedHashMap();
					
					for(VoSirna vo : genelist){
						miRnaList.put(vo.getSiRecords(), vo.getSiRnaSeq());
						geneIdList.put(vo.getGeneId(), vo.getSymbol());
					...
					while (itMirna.hasNext()) {
				        ...
				        resultAssociatedMirna.put(IVoConstants.RESULT_MIRNA, pairs.getKey());
						resultAssociatedMirna.put(IVoConstants.RESULT_MIRNA_SEQ, pairs.getValue());
					...
					while (itGeneId.hasNext()) {
						...
						ArrayList<Integer> pureMirna = new ArrayList();
						
						for(VoSirna vo : genelist){
							if((Integer) pairs.getKey() == vo.getGeneId()){
								mirnaArray.put(vo.getSiRecords(), vo.getSiRecords());
							}
						}
						...
						while (itemMirna.hasNext()) {
							...
							pureMirna.add((Integer) insidePairs.getKey());
						}
					...
					ArrayList<LinkedHashMap> cytoMap = cytoscapeTools.DrawNetworkForSirna(resultAssociatedTargetGene, pathwayArray, ws);
