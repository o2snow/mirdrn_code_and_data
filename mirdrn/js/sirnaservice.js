...
$(function() {	
	...
	token = $("#phpToken").val();
	//==============================================	
	$("#disease").mouseover(function(){
  		$("#disease").attr('src',"img/ncRANppi_icon_disease_over.png");
	});
	$("#disease").mouseout(function(){
    	$("#disease").attr('src',"img/ncRANppi_icon_disease_off.png");
  	});
	...
	$('#export').click(function(e){
		...
		sb = sb + "\n";
		sb = sb + "\n";
		sb = sb + "This file stores all the sub-pathways by specific siRNA regulated."+"\n";
...
function GetDisease(){
		...
		try {
				...
				if(pvalue == ""){
					...
					var pvalueIsOK = isNumber(pvalue);
					if( pvalueIsOK != true || !(pvalue < 1)){
						alert("Please check the P-Value is less than 1");
						return;
					}
				}
				...
				$.getJSON('http://mirdrn.ncu.edu.tw:8080/map/service/GetSirnaMAP?callback=?', {
					token: token,
					sirna: mirna,
					tissue: tissue,			
					kegg: kegg,
					...
					pvalue: pvalue			
						...
						if(data[0].status == 1001){	
							...
							var associated_mirnas = data[0].associated_mirnas;
							if(associated_mirnas != null){	
							
								//Associated_miRNA
								var mirna = data[0].associated_mirnas.mirna;													
								//var outMirnaArray = mirnaArray.join(", ");
								
								var mirna_seq = data[0].associated_mirnas.mirna_seq;
																
								//var outPubmedArray = pubmedArray.join(", ");								
								var htmlAssociatedMirnas = "<tr><td class=\"column_title\">miRNA</td><td class=\"column_title\">miRNA Seq.</td></tr><tr><td width=\"70%\" bgcolor=\"#FFFFFF\" class=\"table_text\">"+mirna+"</td><td width=\"30%\" bgcolor=\"#FFFFFF\" class=\"table_text\">"+mirna_seq+"</td></tr>";
								//$("#assoicated_gene_total").html("Total:&nbsp;"+mirnaArray.length );
								$("#associated_mirnas").html(htmlAssociatedMirnas);
								...
								$.each( targetGeneArray, function( key, val ) {
									...
									outTargetGene = outTargetGene + "<tr><td width=\"25%\" bgcolor=\"#FFFFFF\" class=\"table_text\"><a href=\"http://www.genecards.org/cgi-bin/carddisp.pl?gene="+symbol[1]+"\" target=\"_blank\">"+symbol[1]+"</a></td><td width=\"20%\" bgcolor=\"#FFFFFF\" class=\"table_text\">"+ocgMarker+"</td><td width=\"25%\" bgcolor=\"#FFFFFF\" class=\"table_text\">"+omimTag+"</td><td width=\"30%\" bgcolor=\"#FFFFFF\" class=\"table_text\">"+outBeMirna+"</td></tr>";
								});
...
function ProcessCyto(){	
	...
	$.each( raw_data_edges_array, function( key, val ){
		...
		if(isNumber(raw_data_edges_array[key].target)){
