...
function GetDisease(){
		...
		try {
				...
				// Filter				
				if(disease == ""){			
					alert("Please select one disease");
					return;
				}	
				...
				$.getJSON('http://mirdrn.ncu.edu.tw:8080/map/service/GetDiseaseMap?callback=?', {
					token: token,
					disease: disease,
					tissue: tissue,			
					kegg: kegg,
					...
					pvalue: pvalue			
						...
						if(data[0].status == 1001){	
							...
							if(associated_mirnas != null){	
								...
								//Associated_miRNA
								var mirnaArray = data[0].associated_mirnas.associated_mirnas_mirna;						
								var outMirnaArray = mirnaArray.join(", ");
								
								var pubmedArray = data[0].associated_mirnas.associated_mirnas_pubmed;
								var outPubmedArray = "";
								var num = 0;
								...
								var htmlAssociatedMirnas = "<tr><td class=\"column_title\">miRNA</td><td class=\"column_title\">Pubmed</td></tr><tr><td width=\"70%\" bgcolor=\"#FFFFFF\" class=\"table_text\">"+outMirnaArray+"</td><td width=\"30%\" bgcolor=\"#FFFFFF\" class=\"table_text\">"+outPubmedArray+"</td></tr>";
...
function ProcessCyto(){	
	...
	for(i=0; i< SUBPATHWAY_STRORE_BUFFER.length; i++){
			...
			//counting_sb_genes.push(node1);
			//counting_sb_genes.push(node2);
			//counting_sb_genes.push(node3);
	...
	//var counting_sb_genes_uni = Array.from(new Set(counting_sb_genes));
	//alert(counting_sb_genes_uni.length);
